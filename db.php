...
function get_single_random(){
    global $db;
    $single = $db->query("SELECT * FROM single ORDER BY rand() LIMIT 5");
    return $single;
}
...
function get_single_all_electronics_1(){
    global $db;
    $single = $db->query("SELECT * FROM single WHERE category_id = 1 ORDER BY date DESC LIMIT 2 OFFSET 0");
    return $single;
}

function get_single_all_electronics_2(){
    global $db;
    $single = $db->query("SELECT * FROM single WHERE category_id = 1 ORDER BY date DESC LIMIT 3 OFFSET 2");
    return $single;
}
...
function get_single_all_itnews(){
    global $db;
    $single = $db->query("SELECT * FROM single WHERE category_id = 2 ORDER BY date DESC LIMIT 4 OFFSET 0");
    return $single;
}

function get_single_all_reviews(){
    global $db;
    $single = $db->query("SELECT * FROM single WHERE category_id = 3 ORDER BY date DESC LIMIT 4 OFFSET 0");
    return $single;
}

function get_single_all_videocards(){
    global $db;
    $single = $db->query("SELECT * FROM single WHERE category_id = 4 ORDER BY date DESC LIMIT 4 OFFSET 0");
    return $single;
}
...
function get_single_all_bitcoin_2(){
    global $db;
    $single = $db->query("SELECT * FROM single WHERE category_id = 5 ORDER BY date DESC LIMIT 2 OFFSET 1");
    return $single;
}
